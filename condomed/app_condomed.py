...
PDF_RETENTION_HOURS = 2  # Manter PDFs por 2 horas
CLEANUP_INTERVAL_SECONDS = 600  # Verificar a cada 10 minutos
...
def limpar_pdfs_antigos():
    """Deleta PDFs com mais de 2 horas de idade"""
    try:
        agora = datetime.now()
        limite = agora - timedelta(hours=PDF_RETENTION_HOURS)
        
        for root, dirs, files in os.walk(PASTA_DOCS_PATH):
                ...
                if arquivo.endswith('.pdf'):
                    ...
                    if tempo_modificacao < limite:
                        try:
                            ...
                            print(f"🗑️ Deletado: {caminho_arquivo}")
                        except Exception as e:
                            print(f"❌ Erro ao deletar {caminho_arquivo}: {str(e)}")
        
        # Limpar pastas vazias
        for root, dirs, files in os.walk(PASTA_DOCS_PATH, topdown=False):
            for pasta in dirs:
        ...
        print(f"❌ Erro na limpeza de PDFs: {str(e)}")

def background_cleanup():
    """Roda limpeza de PDFs em background a cada 10 minutos"""
    global CLEANUP_RUNNING
    CLEANUP_RUNNING = True
    print(f"🔄 Background cleanup iniciado (intervalo: {CLEANUP_INTERVAL_SECONDS}s)")
    
    while CLEANUP_RUNNING:
        try:
            time.sleep(CLEANUP_INTERVAL_SECONDS)
            print(f"\n🧹 Verificando PDFs antigos em {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}...")
            limpar_pdfs_antigos()
        except Exception as e:
            print(f"❌ Erro no background cleanup: {str(e)}")
...
def detectar_tipo_documento(pdf_path):
    """Detecta se o documento é FEDCORP ou CONDOMED"""
    try:
        with pdfplumber.open(pdf_path) as pdf:
            ...
            # Detectar CONDOMED (NFS-e)
            if "nfs-e" in texto_lower or "nota fiscal de serviço" in texto_lower:
                return "CONDOMED"
            
            # Detectar FEDCORP (FATURA)
            if "fatura" in texto_lower or "fedcorp" in texto_lower:
                return "FEDCORP"
            
            # Padrão padrão: CONDOMED
            return "CONDOMED"
    except:
        return "CONDOMED"
...
def gerar_remessa_lote(lista_dados, competencia, tipo='CONDOMED'):
    """Gera remessa no formato Ahreas conforme documentação oficial"""
    linhas = []
    
    # Definir dados conforme tipo
    if tipo == "FEDCORP":
    ...
    for dados in lista_dados:
        ...
        if tipo == "FEDCORP":
            # FEDCORP tem campos de impostos
            registro_1 = (
                "1" +                                                    # Tipo
    ...
    <body>
        <div class="container">
            ...
            <div class="card">
                ...
                <div class="form-group">
                    ...
                    <select id="tipoFornecedor" style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 4px;">
                        <option value="CONDOMED">🏢 CONDOMED (NFS-e)</option>
                        <option value="FEDCORP">📋 FEDCORP (Boletos)</option>
                    </select>
                </div>
                <div class="form-group">
        ...
        <script>
            ...
            async function uploadArquivos() {
                ...
                const tipoFornecedor = document.getElementById('tipoFornecedor').value;
                ...
                formData.append('tipo', tipoFornecedor);
...
def api_processar():
    ...
    try:
        ...
        tipo = request.form.get('tipo', 'CONDOMED')
...
def servir_pdf(year, month, filename):
    ...
    try:
        ...
        caminho_completo = os.path.join(caminho_dir, filename)
        
        # Debug logs
        print(f"\n=== DEBUG /docs ===")
        print(f"PASTA_DOCS_PATH: {PASTA_DOCS_PATH}")
        print(f"Year: {year}, Month: {month}, Filename: {filename}")
        print(f"Caminho dir: {caminho_dir}")
        print(f"Caminho completo: {caminho_completo}")
        print(f"Existe? {os.path.exists(caminho_completo)}")
        print(f"Dir existe? {os.path.exists(caminho_dir)}")
        if os.path.exists(caminho_dir):
            print(f"Arquivos em {caminho_dir}: {os.listdir(caminho_dir)}")
        print(f"===================\n")
        
        if os.path.exists(caminho_completo):
            return send_from_directory(caminho_dir, filename, mimetype='application/pdf')
        else:
            return jsonify({"erro": "Arquivo não encontrado", "caminho": caminho_completo}), 404
    except Exception as e:
        print(f"Erro na rota /docs: {str(e)}")
...
if __name__ == '__main__':
    # Iniciar background cleanup thread
